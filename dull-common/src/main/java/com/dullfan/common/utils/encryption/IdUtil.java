...
public class IdUtil {
    ...
    public synchronized static Long get() {
        ...
        if (timestamp < lastTimestamp) {
            ...
            throw new RuntimeException(java.lang.String.format("Clock moved backwards.  Refusing to generate id for %d milliseconds", lastTimestamp - timestamp));
        }
     ...
     * url字符转义
     */
    public static String urlCharacterEscape(String shareId){
        return shareId.replace("%/", "%2F").replace("+", "%2B");
    }
    ...
    public static Long decrypt(String decryptId) {
        decryptId = decryptId.replace("%2F", "/").replace("%2B", "+");
