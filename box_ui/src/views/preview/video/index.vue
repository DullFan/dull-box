        ...
        <div class="video-content">
            ...
            <el-row>
                <el-col :span="18">
                    <!--视频播放容器-->
                    <div class="video-content">
                        <video id="video_player" :src="videoShowPath" autoplay="true" controls="true"/>
                    </div>
                </el-col>
                <el-col :span="6">
...
const renderVideoList = (dataList) => {
    ...
    dataList.forEach((item, index) => {
        ...
        if (item.fileId === route.params.fileId) {
            ...
            videoShowPath.value = boxUtil.getPreviewUrl(item.fileId)
        }
...
.video-content .video-name-content {
    display: block;
    width: 100%;
    text-align: center;
    padding: 10px 0 0 0;
...
.video-content .video-name-content .video-name {
    color: #409EFF;
    font-size: 35px;
    font-weight: bold;
...
.video-content .video-content #video_player {
...
.video-content .video-list-content .video-list {
    width: 100%;
    height: 100%;
    overflow: scroll;
}

.video-content .video-list i {
    margin-right: 15px;
}
