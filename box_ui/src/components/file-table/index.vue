<template>
    <div class="main-file-list-content">
        <el-table
...
const showOffice = (row) => {
    openNewPage('/preview/office', 'PreviewOffice', {
        fileId: boxUtil.handleId(row.fileId)
    })
}
...
const showImg = (row) => {
    ...
    for (let i = 0, iLength = fileList.value.length; i < iLength; ++i) {
        if (fileList.value[i].fileType === 7) {
            imgs.push(boxUtil.getPreviewUrl(fileList.value[i].fileId))
...
const showMusic = (row) => {
    openNewPage('/preview/music', 'PreviewMusic', {
        fileId: boxUtil.handleId(row.fileId),
        parentId: boxUtil.handleId(row.parentId)
    })
}
...
const clickFilename = (row) => {
    switch (row.fileType) {
        case 0:
            goInFolder(boxUtil.handleId(row.fileId))
            break
        case 3:
        case 4:
        case 10:
